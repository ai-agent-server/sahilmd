...
module.exports = {
  ...
  execute: async (conn, message, m, { from, q, isGroup, reply, sender, isEnvOwner }) => {
    ...
    try {
      ...
      // ".promote all" — promote every non-admin member (except the bot itself)
      if ((q || '').trim().toLowerCase() === 'all') {
        ...
        if (targets.length === 0) return send(['Result: ℹ️ Everyone in this group is already an admin.']);

        await conn.groupParticipantsUpdate(from, targets, "promote");
        return send([`Result: ✅ Promoted ${targets.length} member(s) to admin successfully.`]);
      }
      ...
      if (!target) return send(['Error: ❌ Mention or reply to a user to promote.']);

      await conn.groupParticipantsUpdate(from, [target], "promote");
      return send(['Result: ✅ User promoted to admin successfully.']);

    } catch (e) {
      console.error("Promote error:", e);
      return send(['Error: ⚠️ Failed to promote user.']);
    }
  }
};
